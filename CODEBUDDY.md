...
**ChildPCGuard** — Windows 10/11 儿童电脑使用时间管控程序。核心亮点：企业级自我保护能力（12 层防护），对抗有一定电脑基础的初中生。唯一完整参考文档：`docs/ChildPCGuard_项目方案.md`。
...
**构建单个项目**
```powershell
dotnet build src/GuardService/GuardService.csproj
```
...
3. **配置安全**：`config.bin` 使用 AES-256-CBC 加密，密钥存储在注册表（SYSTEM 权限保护）。密码使用 BCrypt 不可逆哈希，明文密码不落盘。
...
| Phase | 内容 | 预估时间 |
|-------|------|----------|
| 1 | MVP：GuardService + TimeTracker + RuleEngine + 系统原生锁屏 + state.json 恢复 | ~2 周 |
| 2 | 防护加固：服务 DACL + ACL + 进程 DACL + NTP 校验 + AES 配置加密 | ~1 周 |
| 3 | 自定义锁屏：LockOverlay + 虚拟桌面 + 键盘钩子 + BCrypt 密码验证 | ~1 周 |
| 4 | 双进程互保：AgentA/AgentB + 心跳机制 + 进程名伪装 | ~3-5 天 |
| 5 | 管理员界面：AdminPanel + Named Pipe 通信 + 全局热键 | ~1 周 |
| 6 | 可选增强：应用黑名单 + 虚拟机检测 + 统计图表 | ~1-2 周 |
